...
import { getLocalStorageItem, removeLocalStorageItem, setLocalStorageItem } from "@/libs/configs/config.helper";
...
export const selectSubtotal = (state: RootState) =>
    state.cart.localCartItems.reduce((sum: number, item: cartItem) => sum + item.currentPrice * item.quantity, 0);
